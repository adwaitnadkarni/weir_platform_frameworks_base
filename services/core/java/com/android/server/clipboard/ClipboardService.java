...
public class ClipboardService extends IClipboard.Stub {
    ...
    //WEIR:
    private WeirManagerInternal weir;
	...
	//WEIR: 
	HashSet<Long> secrecyLabel;
	HashSet<Long> getSecrecyLabel(){
	    return secrecyLabel;
	}
	void setSecrecyLabel(HashSet<Long> secrecyLabel){
	    this.secrecyLabel = secrecyLabel;
	}
	//
    }

    //WEIR: The SparseArray now maps a userId with a set of clipboards, each
    //with a corresponding label.
    private SparseArray<HashSet<PerUserClipboard>> mClipboards = new SparseArray<HashSet<PerUserClipboard>>();
    ...
    private PerUserClipboard getClipboard(int userId) {
	    ...
	    HashSet<Long> callerLabel = getWeir().getProcessLabel(Binder.getCallingPid());
	    PerUserClipboard retpuc = null;
            HashSet<PerUserClipboard> pucs = mClipboards.get(userId);
            if (pucs == null) {
		pucs = new HashSet<PerUserClipboard>();
		retpuc = new PerUserClipboard(userId);
		retpuc.setSecrecyLabel(callerLabel);
		pucs.add(retpuc);
                mClipboards.put(userId, pucs);
            } else{
		for(PerUserClipboard puc: pucs){
		    if(WeirLabelOps.equals(puc.getSecrecyLabel(), callerLabel)){
			return puc;
		    }
		}
		//Clearly not found, must allocate a new one.
		retpuc = new PerUserClipboard(userId);
		retpuc.setSecrecyLabel(callerLabel);
		pucs.add(retpuc);
		mClipboards.put(userId, pucs);
    ...
    //WEIR: Must remove all clipboards for a particular user.
