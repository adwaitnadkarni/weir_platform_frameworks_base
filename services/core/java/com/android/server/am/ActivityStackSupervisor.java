...
public final class ActivityStackSupervisor implements DisplayListener {
	...
	for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
		...
		if (hr != null) {
		    String processIndex = hr.info.applicationInfo.uid+":"+hr.processName;
		    ArrayList<String> existingProcesses = mService.getWeir().getExistingProcesses(processIndex);
		    //if (hr.app == null && app.uid == hr.info.applicationInfo.uid
		    //	    && processName.equals(hr.processName)) {
		    if (hr.app == null && app.uid == hr.info.applicationInfo.uid
			    && (processName.equals(hr.processName) || (existingProcesses!=null && existingProcesses.contains(processName) && WeirLabelOps.equals(hr.getSecrecyLabel(), mService.getWeir().getProcessLabel(app.pid))))) {
			//System.out.println("WEIR_DEBUG: Attaching process "+app+" to activity "+hr);
			try {
	...
	//System.out.println("WEIR_DEBUG: realStartActivityLocked called for "+r+"; in proc "+app);
	...
	//Weir:
	//1. Check if r's label matches the app's label
	//If not, check your other processes for the processname:uid
	//combination from weir and getProcessRecordLocked, for matching
	//labels.  If there is a match, change app to that process.
	//If no match, allocate a new processName (r.processName+"_weir_"+count);
	//Send the new processName via startProcessLocked
	boolean newProcess = false;
	String newProcessName = r.processName;
	if(r.realActivity.equals(mService.getStartingActivity())){
	    //System.out.println("WEIR: in StartSpecificActivityLocked: setting startingProcessName to "+newProcessName);
	    mService.setStartingProcessName(newProcessName);
	}
	if(app!=null){
	    //System.out.println("WEIR_DEBUG: First label comparison- activity:"+r.realActivity+"; processName:"+r.processName+"; pid="+app.pid);
	    app.setSecrecyLabel(mService.getWeir().getProcessLabel(app.pid));
	    if(!WeirLabelOps.equals(app.getSecrecyLabel(), r.getSecrecyLabel())){
		newProcess = true;
	    }
	    //System.out.println("WEIR_DEBUG: First label comparison- activity:"+r.realActivity+"; processName:"+r.processName+"; pid="+app.pid+" match="+!newProcess);
	    if(newProcess){
		...
		//a compatible secrecy context, run this activity in that
		//process.
		String processIndex = r.info.applicationInfo.uid+":"+r.processName;
		ArrayList<String> existingProcesses = mService.getWeir().getExistingProcesses(processIndex);
		if(existingProcesses==null){
			...
			ProcessRecord app_temp = mService.getProcessRecordLocked(proc,
			    r.info.applicationInfo.uid, true);
			//System.out.println("WEIR_DEBUG: checking label of weir process:"+proc+"; real process:"+r.processName+"; realActivity:"+r.realActivity+"; actlabel:"+r.getSecrecyLabel()+"; procrecord:"+app_temp);

			if(app_temp!=null){
				app_temp.setSecrecyLabel(mService.getWeir().getProcessLabel(app_temp.pid));
				//System.out.println("WEIR_DEBUG: Before second label comparison- activity:"+r.realActivity+"; realprocess:"+r.processName+"; weirpid="+app_temp.pid+" proclabel="+app_temp.getSecrecyLabel()+"; act_label="+r.getSecrecyLabel());
				if(WeirLabelOps.equals(app_temp.getSecrecyLabel(), r.getSecrecyLabel())){ 
				    ...
				    newProcess=false;
				    //
				    //must still choose this process in the next
				    //stage (if app.thread is null, it will proceed
				    //to startProcessLocked in mService, where we
				    //will need the startingProcess to be our
				    //chosen one; else we repeat all these efforts.
				    mService.setStartingProcessName(proc);
				    newProcessName=proc;
				    //No need to call setWeirDirForProcess since the process is already alive.
				    break;
				}
			    }
			}
		...
		if(newProcess){
		    newProcessName = mService.getWeir().allocateProcessName(processIndex, r.processName);
	...
	//If the label is not null, create a new subdir for it.
	/*
	if(r.info.applicationInfo.uid!=0 && (r.getSecrecyLabel()!=null && !r.getSecrecyLabel().isEmpty())){
	    //This works, but commenting it for now. Will replace it with an overlayfs hook
	    //createDataDirIfNecessary(r, app);
	}*/
	...
	if(r.info.applicationInfo.uid!=0 && (r.getSecrecyLabel()!=null && !r.getSecrecyLabel().isEmpty()) && (app==null || newProcess)){
	    setWeirDirForProcess(r, newProcessName);
	}
	
	if (app != null && app.thread != null && !newProcess) {
	    try {
	...
	mService.startProcessLocked(newProcessName, r.info.applicationInfo, true, 0,
		"activity", r.intent.getComponent(), false, false, true);
    }

    private void setWeirDirForProcess(ActivityRecord r, String newProcessName){
	//System.out.println("WEIR_DEBUG: setWeirDirForProcess called for activityrec:"+r+"; processName:"+newProcessName);
	...
	String seinfo = r.info.applicationInfo.seinfo;
	//Get the weir_prefix based on the label and packagename
	String weir_prefix = mService.getWeir().getDirectoryPrefix(r.info.applicationInfo.packageName, r.getSecrecyLabel());
	String newDataDir = "";
	...
	if(!newWeirDir.exists()){
	    ...
	    int ret_datadir = mService.mContext.getPackageManager().createDataDirsWeir(packageName, uid, user_id, seinfo, "/weir"+weir_prefix);
	}

	//Setting the processName and data dir, if the process is labeled.
	//
	mService.getWeirDirMap().put(newProcessName, newDataDir);
	//Get the external dir for this label from Weir
	String weirExternalDir = mService.getWeir().getExternalDirectory(r.getSecrecyLabel());
        mService.getWeirExternalDirMap().put(newProcessName, weirExternalDir);

    }


    private void createDataDirIfNecessary(ActivityRecord r, ProcessRecord app){
	//System.out.println("WEIR_DEBUG: createDataDir called for activityrec:"+r+"; procrec:"+app);
	int uid = r.info.applicationInfo.uid;
	int user_id = UserHandle.getUserId(uid);
	String packageName = r.info.applicationInfo.packageName;
	String seinfo = r.info.applicationInfo.seinfo;
	//Get the weir_prefix based on the label and packagename
	String weir_prefix = mService.getWeir().getDirectoryPrefix(r.info.applicationInfo.packageName, r.getSecrecyLabel());
	ApplicationInfo newInfo = new ApplicationInfo(r.info.applicationInfo);
	//Create the datadir as it should be on the fs
	if(user_id == 0){
	    newInfo.dataDir = Environment.getDataDirectory().getAbsolutePath() + File.separator + "data" + File.separator + packageName + File.separator + weir_prefix;
	} else {
	    newInfo.dataDir = Environment.getDataDirectory().getAbsolutePath() + File.separator + "user" + File.separator + user_id + File.separator + packageName + File.separator + weir_prefix;
	}
	File newWeirDir = new File (newInfo.dataDir);
	//System.out.println("WEIR_DEBUG: createDataDir; checking if exists, dir::"+newInfo.dataDir);
	if(!newWeirDir.exists()){
	    //System.out.println("WEIR_DEBUG: createDataDir; does not exist, creating dir::"+newInfo.dataDir);
	    int ret_datadir = mService.mContext.getPackageManager().createDataDirsWeir(packageName, uid, user_id, seinfo, weir_prefix);

	    //WEIR: Not setting the applicationInfo, as we will mount the new dir on the same dir location using overlayfs
	    //i.e., We will mount data/data/pkgname as thee ro lowerlayer in
	    //the mnt namespace of this process, and
	    ///data/data/pkgname/wdir_xx as the upperlayer, on the path
	    ///data/data/pkgname.
	    /*
	    r.info.applicationInfo = newInfo;
	    if(app!=null){//app can be null, if new process is being started.
		//app.info = newInfo;
	    }*/
	    //System.out.println("WEIR::Created Datadir in ActivityManagerService result = " + ret_datadir);
	} else{
	    //WEIR: Not setting the new applicationInfo for the same reason as above.
	    /*r.info.applicationInfo = newInfo;
	    if(app!=null){//app can be null, if new process is being started.
		//app.info = newInfo;
	    }*/
	...
	//Weir:
	//1. get the call pid's label from weir, and set it to r
	//2. Check the label on the intent, and if the callerLabel can change
	//to the label on the intent. If yes, set the callerLabel as the label
	//on the intent. Remove the label from the intent.
	//3. If sourceRecord != null, set it to the sourceRecord too, just for
	//correction.  
	//4. Set the global starting ComponentName, Label, processName to keep
	//state.
	//System.out.println("WEIR: Get caller's processLabel for pid="+callingPid+", realCallingPid="+realCallingPid);
	HashSet<Long> callerLabel = mService.getWeir().getProcessLabel(realCallingPid);
	if(sourceRecord!=null){
	    sourceRecord.setSecrecyLabel(callerLabel);
	}
	...
	if(intentLabelEnabled) {
	    HashSet<String> intentLabel = intent.removeLabel();
	    if(intentLabel!=null){
		//System.out.println("WEIR: Get caller's intentLabel for pid="+callingPid+", realCallingPid="+realCallingPid);
		callerLabel = mService.getWeir().checkIntentCallerLabel(realCallingUid, callerLabel, intentLabel);
	    }
	}
	//

	r.setSecrecyLabel(callerLabel);
	mService.setStartingActivity(r.realActivity);
	//System.out.println("WEIR: Set starting processName to "+r.processName);
	mService.setStartingProcessName(r.processName);
	mService.setStartingLabel(callerLabel);
	//
	...
	if (((launchFlags & Intent.FLAG_ACTIVITY_NEW_TASK) != 0 &&
		...
		//
		// Weir: Hooks are in the component comparison in
		// findTaskLocked and findActivityLocked, in the ActivityStack.
	...
	if (r.packageName != null) {
		...
		if (top.realActivity.equals(r.realActivity) && top.userId == r.userId) {
		    ...
		    //Weir:
		    //1. Get top's label and apply to top. Reuse the existing app!=null check.
		    //2. Also compare its label to r's, and resume top only if equal.
		    //3. If top's process is not alive, nothing happens.
		    if (top.app != null && top.app.thread != null) {
			top.setSecrecyLabel(mService.getWeir().getProcessLabel(top.app.pid));
			if(WeirLabelOps.equals(top.getSecrecyLabel(), r.getSecrecyLabel())){
	...
	if (r.resultTo == null && inTask == null && !addingToTask
		...
		//Weir: Hook in performClearTaskLocked will return only a matching activity record.
		...
		//Weir: Hook in findActivityInHistoryLocked will return only a matching activity record.
		...
		if(top.app != null){
		    ...
		    top.setSecrecyLabel(r.getSecrecyLabel());
		}
		if(WeirLabelOps.equals(top.getSecrecyLabel(), r.getSecrecyLabel())){
		    if ((launchFlags & Intent.FLAG_ACTIVITY_SINGLE_TOP) != 0
			    || launchSingleTop || launchSingleTask) {
