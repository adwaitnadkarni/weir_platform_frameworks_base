...
final class TaskRecord {
    ...
    final ActivityRecord performClearTaskLocked(ActivityRecord newR, int launchFlags) {
		...
		//Weir: TODO: 
		//1. Also compare labels if alive
		//2. If not, set the label of r to newR's label
		if(r.app!=null){
		    r.setSecrecyLabel(mService.getWeir().getProcessLabel(r.app.pid));
		    if(!WeirLabelOps.equals(r.getSecrecyLabel(), newR.getSecrecyLabel())){
			continue;
		    }
		}
		else r.setSecrecyLabel(newR.getSecrecyLabel());
    ...
    final ActivityRecord findActivityInHistoryLocked(ActivityRecord r) {
	    ...
	    //Weir: TODO: 
	    //1. Also compare labels if alive
	    //2. If not, set the label of r
            if (candidate.realActivity.equals(realActivity)) {
		if(candidate.app!=null){
		    candidate.setSecrecyLabel(mService.getWeir().getProcessLabel(candidate.app.pid));
		    if(!WeirLabelOps.equals(candidate.getSecrecyLabel(), r.getSecrecyLabel())){
			continue;
		    }
		}
		else candidate.setSecrecyLabel(r.getSecrecyLabel());
