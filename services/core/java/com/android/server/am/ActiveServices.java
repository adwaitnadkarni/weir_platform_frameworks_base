...
public final class ActiveServices {
	...
	final int mUserId;
	//WEIR: Added the label to mServicesByName and Intent, making the
	//ComponentName and Intent.FilterComparison point to an array of
	//ServiceRecords (with separate labels) instead of a single
	//ServiceRecord.
	final ArrayMap<ComponentName, HashSet<ServiceRecord>> mServicesByName
		= new ArrayMap<ComponentName, HashSet<ServiceRecord>>();
	final ArrayMap<Intent.FilterComparison, HashSet<ServiceRecord>> mServicesByIntent
		= new ArrayMap<Intent.FilterComparison, HashSet<ServiceRecord>>();
	...
	*/
	...
	HashSet<ServiceRecord> records = getServiceMap(callingUser).mServicesByName.get(name);

	//WEIR: Get the callerLabel, and compare labels
	HashSet<Long> callerLabel = mAm.getWeir().getProcessLabel(Binder.getCallingPid());
	int weirPid = mAm.getRealCallingPid();
	if (weirPid >0) {
	    callerLabel = mAm.getWeir().getProcessLabel(weirPid);
	}

	if(records!=null){
	    for (ServiceRecord r : records){
		    if(r.app!=null){
			r.app.setSecrecyLabel(mAm.getWeir().getProcessLabel(r.app.pid));
			r.setSecrecyLabel(r.app.getSecrecyLabel());
		    }
		if(WeirLabelOps.equals(r.getSecrecyLabel(), callerLabel)){
		     return r;
		}
	    }
	}
    ...
    ArrayMap<ComponentName, HashSet<ServiceRecord>> getServices(int callingUser) {
	...
	//WEIR: 1. Get caller process's seclabel, and set it as callerLabel
	HashSet<Long> callerLabel = mAm.getWeir().getProcessLabel(callingPid);
	//Apply the intent's label if possible
	// WEIR: TODO: The parceling logic in the intent label somehow messes things up for the callerLabel. 
	// Fix the stability of this, then enable.
	
	boolean intentLabelEnabled=false;
	if(intentLabelEnabled) {
	    HashSet<String> intentLabel = service.removeLabel();
	    if(intentLabel!=null){
		//System.out.println("WEIR: Get caller's intentLabel for pid="+callingPid+", realCallingPid="+realCallingPid);
		callerLabel = mAm.getWeir().checkIntentCallerLabel(callingUid, callerLabel, intentLabel);
	    }
	}
	//WEIR: We pass this to retrieveServiceLocked, as only then
	//will it give us the appropriate service.  This is different from
	//activities, whose info is queried from the PackageManager instead of
	//records.
	ServiceLookupResult res =
	    retrieveServiceLocked(service, resolvedType,
		    callingPid, callingUid, userId, true, callerFg, callerLabel);
	if (res == null) {
	    return null;
	}
	if (res.record == null) {
	...
	//WEIR: At this point, r.getSecrecyLabel() returns the caller's label.
	...
	//WEIR: If r.app is not null, then retrieveServiceLocked returned an
	//r.app that had a matching label. Else, it returned a new
	//ServiceRecord, in which case r.app == null.
	//
	if (!callerFg && r.app == null && mAm.mStartedUsers.get(r.userId) != null) {

	    ProcessRecord proc = mAm.getProcessRecordLocked(r.processName, r.appInfo.uid, false);
	    //WEIR: TODO: Is proc compatible? If not, set it to null. We might
	    //not want to create a new one now. Lets wait until the system
	    //actually wants to start this service; then we will run the same
	    //test again and create a new process for it.
	...
	//WEIR: 1. Get caller process's seclabel, and set it as callerLabel
	HashSet<Long> callerLabel = mAm.getWeir().getProcessLabel(Binder.getCallingPid());
	//WEIR: Who stops the service? That is, who is authorized? Does the
	//caller know that the service is labeled? If no, then which service do
	//we stop? These are broader questions that apply to activities as
	//well. For now, we stop the activity/service that has the same label
	//as the caller, i.e., belongs to the same application instance.
	//
	// If this service is active, make sure it is stopped.
	ServiceLookupResult r = retrieveServiceLocked(service, resolvedType,
		Binder.getCallingPid(), Binder.getCallingUid(), userId, false, false, callerLabel);
	if (r != null) {
	    if (r.record != null) {
	...
	//WEIR: TODO: 1. Get caller process's seclabel, and set it as callerLabel
	 HashSet<Long> callerLabel = mAm.getWeir().getProcessLabel(Binder.getCallingPid());
	...
	ServiceLookupResult r = retrieveServiceLocked(service, resolvedType,
		...
		UserHandle.getCallingUserId(), false, false, callerLabel);
	...
	//WEIR: Get caller process's seclabel, and set it as callerLabel
	HashSet<Long> callerLabel = mAm.getWeir().getProcessLabel(Binder.getCallingPid());

	ServiceLookupResult res =
	    retrieveServiceLocked(service, resolvedType,
		    Binder.getCallingPid(), Binder.getCallingUid(), userId, true, callerFg, callerLabel);
	if (res == null) {
	    return 0;
	}
	if (res.record == null) {
	...
	try {
		...
		if (bringUpServiceLocked(s, service.getFlags(), callerFg, false) != null) {
		    //System.out.println("WEIR_DEBUG: bindService returning after bringing up service: "+s);
		    return 0;
		}
	    }

	    //System.out.println("WEIR_DEBUG: bindService, after bringUpServiceLocked for: "+s);
	    if (s.app != null) {
		//System.out.println("WEIR_DEBUG: bindService, s.app is not null, for: "+s);
		if ((flags&Context.BIND_TREAT_LIKE_ACTIVITY) != 0) {
    ...
    private ServiceLookupResult retrieveServiceLocked(Intent service,
	    ...
	    boolean createIfNeeded, boolean callingFromFg, HashSet<Long> callerLabel) {
	...
	//Getting the real pid
	//TODO: Enable this, remove the individual getProcessLabel calls in startService, bindService, etc.
	/*
	int weirPid = mAm.getRealCallingPid();
	if (weirPid >0) {
	    callerLabel = mAm.getWeir().getProcessLabel(weirPid);
	}*/
	//System.out.println("WEIR_DEBUG: retrieveServiceLocked for callingPid:"+callingPid+"; callingUid:"+callingUid+"; realPid:"+weirPid+"; callerLabel="+callerLabel);
	//
	if (comp != null) {
	    //
	    //WEIR: This method may be called to stop the service; will it
	    //lead to dangling state if the process raises its label and there
	    //is a mismatch here? Not really; it will just need another process
	    //with a similar state to stop it.
	    r = null;
	    HashSet<ServiceRecord> records = smap.mServicesByName.get(comp);
	    if(records!=null){
		for(ServiceRecord rec: records){
		    if(rec.app!=null){
			rec.app.setSecrecyLabel(mAm.getWeir().getProcessLabel(rec.app.pid));
			rec.setSecrecyLabel(rec.app.getSecrecyLabel());
		    }
		    if(WeirLabelOps.equals(rec.getSecrecyLabel(), callerLabel)){
			r = rec;
			break;
		    }
		}
	...
	if (r == null) {
	    ...
	    HashSet<ServiceRecord> records = smap.mServicesByIntent.get(filter);
	    if(records!=null){
		for(ServiceRecord rec: records){
		    if(rec.app!=null){
			rec.app.setSecrecyLabel(mAm.getWeir().getProcessLabel(rec.app.pid));
	...
	if (r == null) {
		...
		r = null;
		HashSet<ServiceRecord> records = smap.mServicesByName.get(name);
		if(records!=null){
		    for(ServiceRecord rec: records){
			if(rec.app!=null){
			    rec.app.setSecrecyLabel(mAm.getWeir().getProcessLabel(rec.app.pid));
		...
		if (r == null && createIfNeeded) {
		    ...
		    //WEIR: Set the label of the new serviceRecord. Then add it to mServicesByLabel.
		    r = new ServiceRecord(mAm, ss, name, filter, sInfo, callingFromFg, res);
		    //WEIR: r.setSecrecyLabel() to the caller's label
		    r.setSecrecyLabel(callerLabel);
		    //
		    res.setService(r);
		    //WEIR: Now add r to mServicesByName and mServicesByIntent
		    //smap.mServicesByName.put(name, r);
		    HashSet<ServiceRecord> recs = smap.mServicesByName.get(name);
		    if(recs==null){
			recs = new HashSet<ServiceRecord>();
		    }
		    recs.add(r);
		    smap.mServicesByName.put(name, recs);
		    //
		    //smap.mServicesByIntent.put(filter, r);
		    recs = smap.mServicesByIntent.get(filter);
		    if(recs==null){
			recs = new HashSet<ServiceRecord>();
		    }
		    recs.add(r);
		    smap.mServicesByIntent.put(filter, recs);
		    //
		    // Make sure this component isn't in the pending list.
		    for (int i=mPendingServices.size()-1; i>=0; i--) {
			ServiceRecord pr = mPendingServices.get(i);
			//WEIR: Add label check, to make sure we are consistent
			if (pr.serviceInfo.applicationInfo.uid == sInfo.applicationInfo.uid
				&& pr.name.equals(name) && WeirLabelOps.equals(pr.getSecrecyLabel(), callerLabel)) {
	...
	//WEIR: TODO: Instead of equals, check contains. 
	//if (smap.mServicesByName.get(r.name) != r) {
	if (smap.mServicesByName.get(r.name) == null || !smap.mServicesByName.get(r.name).contains(r)) {
	...
	//WEIR: Similar to startSpecificActivity. Must match the service
	//with the right process.  r.app will be null only if
	//retrieveServiceLocked returned it that way, i.e., if a compatible
	//process was not available. In that case, the call should not have
	//reached here.
	if (r.app != null && r.app.thread != null) {
	...
	String newProcessName = r.processName;

	if (!isolated) {
	    app = mAm.getProcessRecordLocked(procName, r.appInfo.uid, false);
	    //WEIR: Do we want to use the default process? If not, we create another one.
	    boolean newProcess = false;
	    //mAm.setStartingService(r.name); //Not used anymore.
	    mAm.setStartingProcessName(newProcessName);
	    mAm.setStartingLabel(r.getSecrecyLabel());
	    if(app!=null){
		//System.out.println("WEIR_DEBUG: First label comparison- service:"+r.name+"; processName:"+r.processName+"; pid="+app.pid);
		app.setSecrecyLabel(mAm.getWeir().getProcessLabel(app.pid));
		if(!WeirLabelOps.equals(app.getSecrecyLabel(), r.getSecrecyLabel())){
		    newProcess = true;
		}
		//System.out.println("WEIR_DEBUG: First label comparison- service:"+r.name+"; processName:"+r.processName+"; pid="+app.pid+" match="+!newProcess);
		if(newProcess){
		    ...
		    //process.
		    String processIndex = r.appInfo.uid+":"+r.processName;
		    ArrayList<String> existingProcesses = mAm.getWeir().getExistingProcesses(processIndex);
		    if(existingProcesses==null){
                    //newProcess remains true
		    }
		    else{
			for(String proc: existingProcesses){
		            ProcessRecord app_temp = mAm.getProcessRecordLocked(proc, r.appInfo.uid, true);
			    //System.out.println("WEIR_DEBUG: checking label of weir process:"+proc+"; real process:"+r.processName+"; service:"+r.name+"; serv_label:"+r.getSecrecyLabel()+"; procrecord:"+app_temp);

			    if(app_temp!=null){
				app_temp.setSecrecyLabel(mAm.getWeir().getProcessLabel(app_temp.pid));
				//System.out.println("WEIR_DEBUG: Before second label comparison- service:"+r.name+"; realprocess:"+r.processName+"; weirpid="+app_temp.pid+" proclabel="+app_temp.getSecrecyLabel()+"; serv_label="+r.getSecrecyLabel());
				if(WeirLabelOps.equals(app_temp.getSecrecyLabel(), r.getSecrecyLabel())){
				    ...
				    mAm.setStartingProcessName(proc);
				    newProcessName=proc;
				    break;
				}
			    }
		    ...
		    if(newProcess){
			newProcessName = mAm.getWeir().allocateProcessName(processIndex, r.processName);
			app = null;
			//System.out.println("WEIR_DEBUG: allocating new process for "+r.processName+"; new="+newProcessName);
			mAm.setStartingProcessName(newProcessName);
		    }
		}
	    } 
	    ...
	    if(r.appInfo.uid!=0 && (r.getSecrecyLabel()!=null && !r.getSecrecyLabel().isEmpty()) && app==null){
		setWeirDirForProcess(r, newProcessName);
	    } 
	...
	if (app == null) {
	    //WEIR: Replace procName with newProcessName
	    //if ((app=mAm.startProcessLocked(procName, r.appInfo, true, intentFlags,
	    if ((app=mAm.startProcessLocked(newProcessName, r.appInfo, true, intentFlags,
		...
		Slog.w(TAG, msg);
		//System.out.println("WEIR_DEBUG: Bringing Down service, as process creation failed. "+r);
		bringDownServiceLocked(r);
		return msg;
	    }
	    if (isolated) {
	...
	if (!mPendingServices.contains(r)) {
	    //System.out.println("WEIR_DEBUG: Adding to pending services: "+r);
	...
	if (r.delayedStop) {
		...
		//System.out.println("WEIR_DEBUG: in bringUpServiceLocked: Asked to Stop: "+r);
		stopServiceLocked(r);
	    }
	}
    ...
    private void setWeirDirForProcess(ServiceRecord r, String newProcessName){
        //System.out.println("WEIR_DEBUG: setWeirDirForProcess called for servrec:"+r+"; processName:"+newProcessName);
        ...
        String weir_prefix = mAm.getWeir().getDirectoryPrefix(r.appInfo.packageName, r.getSecrecyLabel());
        String newDataDir = "";
        ...
        if(!newWeirDir.exists()){
            ...
            int ret_datadir = mAm.mContext.getPackageManager().createDataDirsWeir(packageName, uid, user_id, seinfo, "/weir"+weir_prefix);
        }

        //Setting the processName and data dir in the AMS. Will be used later during process creation for mounting overlayfs.
        mAm.getWeirDirMap().put(newProcessName, newDataDir);
	//Get the external dir for this label from Weir
	String weirExternalDir = mAm.getWeir().getExternalDirectory(r.getSecrecyLabel());
        mAm.getWeirExternalDirMap().put(newProcessName, weirExternalDir);
	...
	//System.out.println("WEIR_DEBUG: realStartServiceLocked called for service: "+r+" in process:"+app);
	if (app.thread == null) {
	...
	//System.out.println("WEIR_DEBUG: sendServiceArgsLocked called for: "+r);
	...
	final ServiceMap smap = getServiceMap(r.userId);
	//WEIR: TODO: Remove specific serviceRecord object. Assuming this is
	//the same object that was added to mServicesByName, we do not have to
	//check labels.  
	//smap.mServicesByName.remove(r.name);
	HashSet<ServiceRecord> records = smap.mServicesByName.get(r.name);
	if(records!=null){
	    records.remove(r);
	    if(records.isEmpty()){ 
		smap.mServicesByName.remove(r.name);
	...
	records = smap.mServicesByIntent.get(r.intent);
	if(records!=null){
	    records.remove(r);
	    if(records.isEmpty()){ 
		smap.mServicesByIntent.remove(r.intent);
	...
	//System.out.println("WEIR_DEBUG: attachApplicationLocked called for: "+proc);
	// Collect any services that are waiting for this process to come up.
	if (mPendingServices.size() > 0) {
	    ServiceRecord sr = null;
	    try {
		//WEIR: We could not change sr's processName as it was final,
		//and we did not know at the time of its creation if we would
		//actually need another processName.
		//Therefore, for the sr's processName, get the actual set of all processNames from Weir.
		//Then, check if sr's processName is IN the list.
		for (int i=0; i<mPendingServices.size(); i++) {
		    sr = mPendingServices.get(i);
		    //WEIR: Get sr's processNames from Weir.
		    String processIndex = sr.appInfo.uid+":"+sr.processName;
		    ArrayList<String> existingProcesses = mAm.getWeir().getExistingProcesses(processIndex);
		    //
		    if (proc != sr.isolatedProc && (proc.uid != sr.appInfo.uid
			    || (!processName.equals(sr.processName) && existingProcesses!=null && !existingProcesses.contains(processName)))) {
			continue;
		    }
    ...
    private boolean collectForceStopServicesLocked(String name, int userId,
	    ...
	    ArrayMap<ComponentName, HashSet<ServiceRecord>> services,
	    ArrayList<ServiceRecord> result) {
	boolean didSomething = false;
	for (int i=0; i<services.size(); i++) {
	    //WEIR: Do this for every service
	    HashSet<ServiceRecord> records = services.valueAt(i);
	    for(ServiceRecord service : records){
	...
	if (userId == UserHandle.USER_ALL) {
	    ...
	    if (smap != null) {
		ArrayMap<ComponentName, HashSet<ServiceRecord>> items = smap.mServicesByName;
		didSomething = collectForceStopServicesLocked(name, userId, evenPersistent,
			doit, items, services);
	    }
	}
	...
	ArrayList<ServiceRecord> services = new ArrayList<ServiceRecord>();
	ArrayMap<ComponentName, HashSet<ServiceRecord>> alls = getServices(tr.userId);
	for (int i=0; i<alls.size(); i++) {
	    //WEIR: For all services. Note that since sr.packageName is
	    //accessed, sr is clearly not expected to be null. We still check
	    //regardless.
	    HashSet<ServiceRecord> records = alls.valueAt(i);
	...
	for (int i=app.services.size()-1; i>=0; i--) {
	    ...
	    //WEIR: Check if the service listed in the app (process) is in the map.
	   // final ServiceRecord curRec = smap.mServicesByName.get(sr.name);
	    //if (curRec != sr) {
	    if(smap.mServicesByName.get(sr.name) != null && !smap.mServicesByName.get(sr.name).contains(sr)){
		/*if (curRec != null) {
	...
	try {
		...
		for (int ui=0; ui<users.length && res.size() < maxNum; ui++) {
		    ArrayMap<ComponentName, HashSet<ServiceRecord>> alls = getServices(users[ui]);
			...
			for(ServiceRecord sr: records){
			    ...
			    //polyinstantiated.
			    if(Binder.getCallingUid()!=android.os.Process.myUid() && !sr.getSecrecyLabel().isEmpty()){
				continue;
			    }
			    //
		...
		for (int i=0; i<mRestartingServices.size() && res.size() < maxNum; i++) {
		    ...
		    //from guessing if a process/component has been
		    //polyinstantiated.
		    if(Binder.getCallingUid()!=android.os.Process.myUid() && !r.getSecrecyLabel().isEmpty()){
			continue;
		    }
		    ActivityManager.RunningServiceInfo info =
		...
		ArrayMap<ComponentName, HashSet<ServiceRecord>> alls = getServices(userId);
		for (int i=0; i<alls.size() && res.size() < maxNum; i++) {
		    //WEIR: All services
		    HashSet<ServiceRecord> records = alls.valueAt(i);
		    if(records==null)	continue;
	...
	try {
		...
		if (smap.mServicesByName.size() > 0) {
			...
			//WEIR: TODO: Modify to dump unlabeled items. Currently prints the first item.
			//ServiceRecord r = smap.mServicesByName.valueAt(si);
			ServiceRecord r = null;
			HashSet<ServiceRecord> records = smap.mServicesByName.valueAt(si);
			if(records!=null){
			    for(ServiceRecord rec: records){
				r = rec;
			    }	
	...
	synchronized (mAm) {
		...
		for (int user : users) {
		    ...
		    //WEIR: TODO: Dump all unlabeled; for now dump the first.
		    //ArrayMap<ComponentName, ServiceRecord> alls = smap.mServicesByName;
		    ArrayMap<ComponentName, HashSet<ServiceRecord>> alls = smap.mServicesByName;
		    for (int i=0; i<alls.size(); i++) {
			HashSet<ServiceRecord> records = alls.valueAt(i);
			for(ServiceRecord r1: records){
			    services.add(r1);
			    break;
			}
		    }
		...
		for (int user : users) {
		    ...
		    //WEIR: TODO: Dump all unlabeled, for now dump first
		    //ArrayMap<ComponentName, ServiceRecord> alls = smap.mServicesByName;
		    ArrayMap<ComponentName, HashSet<ServiceRecord>> alls = smap.mServicesByName;
		    for (int i=0; i<alls.size(); i++) {
