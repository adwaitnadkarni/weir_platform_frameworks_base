...
final class ActivityStack {
	...
	for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
	    ...
	    //Weir:
	    //1. Check if r's process is alive, if yes, get its label (and set it to r)
	    //2. Compare its label with the label of target; if equal, do nothing.
	    //3. If unequal, continue, i.e., no match.
	    //TODO: Do we need to do the following? Not for now. 
	    //4. Note that if r's process is dead, it must start with the target's label, if it ever returns r.
	    //5. So just before the return statement (s), set r's label to that of the target's.
	    if(r.app != null){
		r.setSecrecyLabel(mService.getWeir().getProcessLabel(r.app.pid));
		if(!WeirLabelOps.equals(r.getSecrecyLabel(), target.getSecrecyLabel())){
		    continue;
		}
	    } 
	...
	for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
		...
		if (!r.finishing && r.intent.getComponent().equals(cls) && r.userId == userId) {
		    ...
		    //
		    //Weir:
		    //1. get the current starting activity and its label, from the global state saved previously.
		    //2. if r's process is alive, then get its label, apply to r, and compare. continue on unmatched label.
		    //3. if r's process is dead, apply starting act's label to it before the return statement.
		    if(r.app != null && mService.getStartingActivity().equals(cls)){
			r.setSecrecyLabel(mService.getWeir().getProcessLabel(r.app.pid));
			if(!WeirLabelOps.equals(r.getSecrecyLabel(), mService.getStartingLabel())){
			    continue;
			}
		    }
		    return r;
	...
	//Weir:
	//1. If the next realActivity is the starting realActivity
	//1.a If next's process is alive, get its label, and apply to next. 
	//1.b Resume it only when the label and the global starting label match; 
	//1.c If you need to restart the activity, set its label to
	//startingLabel, so that the restarted activity will have the correct
	//label.
	boolean weirRestartActivity=false;//must restart if true.
	if(next.realActivity.equals(mService.getStartingActivity())){
	    HashSet<Long> startingSecrecyLabel = mService.getStartingLabel();
	    if(next.app!=null){
		next.setSecrecyLabel(mService.getWeir().getProcessLabel(next.app.pid));
		if(!WeirLabelOps.equals(next.getSecrecyLabel(), startingSecrecyLabel)){
		    weirRestartActivity=true;
		    next.setSecrecyLabel(startingSecrecyLabel);
		}
	    } else{//app == null, must restart anyways
		next.setSecrecyLabel(startingSecrecyLabel);
	    }
	}
	//
	if (next.app != null && next.app.thread != null && !weirRestartActivity) {
