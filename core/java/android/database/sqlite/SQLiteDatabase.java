...
public final class SQLiteDatabase extends SQLiteClosable {
    ...
    public long insertWithOnConflict(String table, String nullColumnHack,
        ...
        try {
            ...
            try {
		//WEIR: TODO:
		//Weir gets called here, with the table, initialValues, statement.
		//Weir 1) gets the caller's label from pid
		//     2) initializes the label arg. in the statement.
		//     3) statement.executeInsert();
		//     4) applies (unions) the caller's label to the database
		//     file (to protect against direct file access) Weir gets
		//     called as we do not want the entire file's label to
		//     propagate to the caller in case of a query.
		//     TODO: May be Weir does not need to be called in case of
		//     an "insert, update" or "delete" operation, and all the
		//     tasks said above can happen in the caller's context?
		//     Only for query we need to call Weir to prevent false
		//     positives.  
		//     NOTE: Make sure we filter the "_label_" field from the
		//     statement.
