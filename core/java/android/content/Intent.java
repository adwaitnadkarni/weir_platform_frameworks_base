...
public class Intent implements Parcelable, Cloneable {
   ...
   /**
    ...
    //WEIR: weirLabel is set by the caller who supplies the intent. The AMS
    //interprets the weirLabel based on the caller's secrecy context, using the "label change" rule
    private HashSet<String> weirLabel;
	...
	if(o.weirLabel!=null){
	    this.weirLabel = new HashSet<String>(o.weirLabel);
	}
	if (o.mExtras != null) {
    ...
    ///WEIR: NOTE: If weirLabel is NULL, it is unset. If it is isEmpty(), it is
    //actually set to empty on purpose.
    public void setEmptyLabel(){
	weirLabel = new HashSet<String>();
    }

    public void addToLabel (String ownerPackageName, String tagName){
	String absoluteTagName = ownerPackageName+":"+tagName;
	if(weirLabel==null){
	    weirLabel = new HashSet<String>();
	}
	weirLabel.add(absoluteTagName);
    }

    //WEIR: Removes and returns the label.
    public HashSet<String> removeLabel() {
	HashSet<String> retWeirLabel = weirLabel;
	weirLabel = null;
	return retWeirLabel;
    }
	...
	//WEIR: 
	if(weirLabel != null) {
	    final int N_label = weirLabel.size();
	    out.writeInt(N_label);
	    for(String label : weirLabel){
	...
	//WEIR: 
	int N_label = in.readInt();
	if (N > -1) {
	    weirLabel = new HashSet<String>();
	    int i;
	    for (i=0; i<N_label; i++) {
		weirLabel.add(in.readString());
	    }
	} else {
	    weirLabel = null;
	}
